...
class Crontab(object):
    ...
    def __init__(self, **options):
        ...
        self.cron_jobs = CronJob.objects.filter(active=True)
    ...
    def add_jobs(self):
        """
        Adds all jobs defined in CronJob objects to internal buffer
        """
        for job_object in self.cron_jobs:
    ...
    def remove_jobs(self):
        """
        Removes all jobs defined in CronJob objects from internal buffer
        """
        for line in self.crontab_lines[:]:
            job = self.settings.CRONTAB_LINE_REGEXP.findall(line)
            if job and job[0][4] == self.settings.CRONTAB_COMMENT:
    ...
    def run_job(self, job_hash):
        """
        Executes the corresponding function defined in CronJob objects
        """
    ...
    def __get_job_by_hash(self, job_hash):
        ...
        for job_object in self.cron_jobs:
            ...
            if self.__hash_job(job) == job_hash:
                return job
        raise RuntimeError(
            'No job with hash %s found. It seems the crontab is out of sync with CronJob objects. '
